...
void PWM_Init(void)
{
	...
	//clear on compare mattch time and set at the bottom
	Set_Bit(TCCR1A, 5);
	Clear_Bit(TCCR1A, 4);
	// Set Prescaler to 64
	Clear_Bit(TCCR1B, 2);
	Set_Bit(TCCR1B, 1);
	Set_Bit(TCCR1B, 0);
	// time period not important because if OCR1B is 0 the time period will be all low
	// if 128 it will be half high and half low 
	// if 255 it will be all high
}
