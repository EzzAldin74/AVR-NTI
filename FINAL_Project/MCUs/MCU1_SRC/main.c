	...
	while (1)
	{
		...
		else if(pressed_key=='=')
		{
			...
			if(num==951236)
			{
				wrong_PIN=0;
				// 0 = Unlocked, 1 = Locked
				u8 door_state = 0 , door2_state = 0;   
				// 0 = OFF, 1 = ON   
				u8 fan_state = 0;     
				// 0 = Sensors Control, 1 = User Controls  
				u8 Sensors = 0; 
				...
				while(1)
				{
					...
					//for refresh the LCD every 500ms until the user click 0
					u8 key = 0xFF;
					...
					if (key == '0')
					{
						...
						while (in_menu)
						{
							...
							switch (choice)
							{
								case '1': // Door Sub-Menu
								{
									...
									while (in_door_menu)
									{
										...
										switch (door_choice)
										{
											...
											TIMER1_voidInitServo();
											//if unlock okay let's lock it 
											if (door_state == 0) {
												TIMER1_voidSetServoAngle(0); 
												door_state = 1;
												LCD_SendCommand(0x01); LCD_SendString(" DOOR 1 LOCKED! ");
											}
											else //if lock okay let's unlock it
											{
												TIMER1_voidSetServoAngle(90);
												door_state = 0;
												LCD_SendCommand(0x01); LCD_SendString(" DOOR 1 UNLOCKED");
											}
											//wait 1sec then open or close the door
											_delay_ms(1000);
											// servo initlization clears and set bits in the fan register 
											// after close or open the door return the fan its bits
											PWM_Init();
											break;
											
											case '2': // Toggle Door 2 (AVR 2 via UART) if unlock okay let's lock it
											if (door2_state == 0) {
								...
								//if fan on okay let's turn it off
								if (fan_state == 1) {
									Motor_SetSpeed(PWM_OFF);
									fan_state = 0;
									LCD_SendCommand(0x01); LCD_SendString("    FAN OFF     ");
								}
								else {//if fan off okay let's turn it on
								...
								if(door_state == 0 && door2_state == 0 )
								 LCD_SendString("UNLCK");
								else 
								LCD_SendString("LOCK ");
...
//generate one pluse to the EXTI
void Light_Switch(u8 mode)
{
	Light_mode=mode;
